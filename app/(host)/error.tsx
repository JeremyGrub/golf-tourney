...
export default function HostError({
  ...
  useEffect(() => {
    ...
    console.error("[thru.] host error:", error);
  }, [error]);

  return (
    <HostChrome email={null}>
      <div className="mx-auto w-full max-w-3xl px-6 py-16 md:px-10">
        <div className="rounded-[28px] border border-ink/10 bg-chalk p-8 md:p-12">
          ...
          <h1 className="font-display text-4xl font-semibold leading-[0.95] tracking-tight md:text-5xl">
            That action didn&apos;t take.
          </h1>
          <p className="mt-4 max-w-md text-ink/70">
            The host console hit an error. Your tournament data is safe — try
            again, or jump back to the dashboard if it keeps happening.
          </p>
          {error.digest && (
